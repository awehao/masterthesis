...
def compute_metrics(messages, goal_tol_m: float = GOAL_TOLERANCE_M) -> dict:
    ...
    out = {
        ...
        'min_clearance_dyn_m'    : float('nan'),
        'min_clearance_static_m' : float('nan'),
        'collision_count'   : 0,
        'collided'          : False,
    ...
    if gt_tracks or static_clr is not None:
        ...
        clearances, n_coll = [], 0
        # Kept apart because they are different failures with different owners:
        # a mover the CBF had to dodge, versus a wall the planner's inflation
        # and the static CBF margin were supposed to keep the robot away from.
        # Measured on pose set A, 30 trials: 8 negative-clearance trials, of
        # which 1 was a mover and 7 were static. Reporting only the minimum of
        # the two hid that split entirely.
        clr_dyn, clr_sta = [], []
        in_collision = False
        for i in np.where(safe_in_run)[0]:
            ...
            d_min = 9.9
            d_dyn = 9.9          # movers only
            d_sta = 9.9          # walls, map boxes, unknown_obs
            ...
            for ots, oxy, oname in gt_tracks:
                ...
                d_dyn = min(d_dyn, surf - ROBOT_RADIUS_M)
            ...
            if static_clr is not None:
                ...
                d_sta = min(d_sta, static_clr(rx, ry) - ROBOT_RADIUS_M)
            clearances.append(float(d_min))
            clr_dyn.append(float(d_dyn))
            clr_sta.append(float(d_sta))
    ...
    if clearances:
        ...
        if clr_dyn:
            v = np.array(clr_dyn); v = v[v < 9.0]
            out['min_clearance_dyn_m'] = float(v.min()) if len(v) else float('nan')
        if clr_sta:
            v = np.array(clr_sta); v = v[v < 9.0]
            out['min_clearance_static_m'] = float(v.min()) if len(v) else float('nan')
...
CSV_HEADER = [
    ...
    'min_clearance_m', 'min_clearance_dyn_m', 'min_clearance_static_m',
    'collision_count', 'collided', 'clearance_source',
