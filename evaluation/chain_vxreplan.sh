...
echo "[$(date +%T)] waiting for the vx sweep ..."
while ! grep -qE "vx symmetric sweep complete|FATAL" evaluation/logs/chainVx.log 2>/dev/null; do sleep 120; done
while [ -f "$LOCKFILE" ] && kill -0 "$(cat "$LOCKFILE" 2>/dev/null)" 2>/dev/null; do sleep 60; done
sleep 30

# The env forwarding list lives in a script bash reads line by line while a
# batch runs, so it can only be edited between batches -- which is here.
...
if ! grep -q REPLAN "$R"; then
    ...
    bash -n "$R" || { echo "FATAL: sed broke run_omnibot_dynamic.sh"; exit 1; }
    grep -q REPLAN "$R" || { echo "FATAL: REPLAN not forwarded"; exit 1; }
...
run_one () {                     # $1 outdir  $2 poses file
    ...
    env POSES_CSV="$PWD/$poses" BIGARENA=1 TRAJ=bigarena_traffic GUI=0 \
        ...
        CBF_PRUNE_RANGE=1.2 CBF_FAR_STRIDE=2 REPLAN=1.0 \
...
run_one vxreplan_A evaluation/results/bigarena_poses.csv
run_one vxreplan_B evaluation/results/bigarena_poses_b.csv
echo "[$(date +%T)] vx+replan sweep complete"
