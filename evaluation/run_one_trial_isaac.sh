#!/usr/bin/env bash
# Run a single benchmark trial in ISAAC SIM (one method, one seed) end-to-end.
#
# Identical to run_one_trial.sh except that the simulator is Isaac Sim instead
# of Gazebo. The controller stack, AMCL reset, recording, goal, watcher and
# teardown are the same, so a difference in results is a difference in the
# simulator. Do not edit one of the two scripts without the other.
#
# Steps (all in this single script, no manual intervention):
#   1.  Launch Isaac Sim (isaac_bench_sim.py) + isaac_dynamic.launch.py
...
#
# Output:
#   bags/isaac_<METHOD>__seed<SEED>/
#   logs/isaac_<METHOD>__seed<SEED>.log
#
# Notes:
...
# Isaac ships its own interpreter; it is the same Python 3.12 as Jazzy, so
# rclpy imports straight from /opt/ros and the sim process can be a ROS node.
ISAAC_PY="${ISAAC_PY:-$HOME/venvs/isaacsim-6.0.1/bin/python}"
if [ ! -x "$ISAAC_PY" ]; then
    echo "ERROR: Isaac python not found at $ISAAC_PY (override with ISAAC_PY=)"
    exit 1
fi
...
# --------------------------------------------------------------------- 1. Isaac
# Isaac needs ~100 s to start (extension load + shader cache) against Gazebo's
# ~15 s, so this waits for /clock to actually appear rather than sleeping a
# fixed amount and hoping. Every downstream node runs on use_sim_time, and a
# node that starts before the clock exists sits with a zero time it never
# recovers from.
ISAAC_SIM_DURATION="${ISAAC_SIM_DURATION:-$(( DURATION + 120 ))}"
echo "[$(date +%T)] [1/7] launching Isaac Sim (sim budget ${ISAAC_SIM_DURATION}s) ..."
"$ISAAC_PY" "${HERE}/isaac_bench_sim.py" \
    --duration "$ISAAC_SIM_DURATION" --rtf 1.0 \
    --out "${HERE}/results/isaac_bench_${METHOD_TAG}__${RUN_TAG}.json" \
    >> "$LOG_FILE" 2>&1 < /dev/null &
PIDS+=( $! )

echo "[$(date +%T)] [1/7] waiting for /clock from Isaac (max 240 s) ..."
CLOCK_OK=0
for i in $(seq 1 240); do
    if timeout 2 ros2 topic echo --once /clock rosgraph_msgs/msg/Clock \
            > /dev/null 2>&1; then
        echo "[$(date +%T)] [1/7]   /clock alive after ${i}s"
        CLOCK_OK=1
        break
    fi
    sleep 1
...
if [ "$CLOCK_OK" -ne 1 ]; then
    echo "[$(date +%T)] [1/7] ERROR: no /clock from Isaac — aborting trial"
    exit 1
fi

echo "[$(date +%T)] [1/7] launching non-simulator nodes (isaac_dynamic.launch.py) ..."
ros2 launch ammr_bringup isaac_dynamic.launch.py \
    >> "$LOG_FILE" 2>&1 < /dev/null &
PIDS+=( $! )
sleep 15
