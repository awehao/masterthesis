...
if [ -z "${ROS_DOMAIN_ID:-}" ]; then
  ...
  echo "  ROS_DOMAIN_ID=95 bash evaluation/run_wb_arm.sh"
  exit 64
fi
export ROS_DOMAIN_ID            # 所有子程序沿用同一值

RUN_ID="${RUN_ID:-wb_arm_$(date +%H%M%S)}"
...
say "=== arm 單動測試 RUN_ID=$RUN_ID domain=$ROS_DOMAIN_ID ==="
...
say "[7/8] 有界命令源（**arm 模式**：joint2 ${ARM_RATE} rad/s；底盤三分量恆 0）"
say "  起始底盤 yaw ${YAW}°（arm 模式不下底盤命令，記錄供 C 判準對照）"
timeout 120 python3 -u evaluation/wb_bounded_cmd.py --mode arm --out "$DIR" \
    --arm-rate "$ARM_RATE" --expect-base-yaw-deg "$YAW" 2>&1 | tee -a "$LOG"
...
python3 -u evaluation/wb_arm_check.py --run "$DIR" 2>&1 | tee -a "$LOG"
CHECK=${PIPESTATUS[0]}
say "輸出 $DIR"
if [ "$CHECK" != "0" ]; then
  say "**判定未通過 —— 不進 sync**（明細見 arm_check.json）"
  exit 6
fi
say "判定通過 —— 可進 sync"
