...
if [ -z "${ROS_DOMAIN_ID:-}" ]; then
  ...
  echo "  ROS_DOMAIN_ID=95 bash evaluation/run_wb_base.sh"
  exit 64
fi
...
say "=== base 介面測試 RUN_ID=$RUN_ID domain=$ROS_DOMAIN_ID ==="
...
say "[3/7] 等 /clock 前進（剩餘準備時間 $(prep_left)s）"
...
say "[7/7] 有界命令源（report frame +x，起始 yaw ${YAW}°）"
timeout 120 python3 -u evaluation/wb_bounded_cmd.py --mode base --out "$DIR" \
