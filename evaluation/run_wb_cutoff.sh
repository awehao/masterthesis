...
if [ -z "${ROS_DOMAIN_ID:-}" ]; then
  ...
  echo "  ROS_DOMAIN_ID=95 bash evaluation/run_wb_cutoff.sh"
  exit 64
fi
export ROS_DOMAIN_ID            # 所有子程序沿用同一值

RUN_ID="${RUN_ID:-wb_cutoff_$(date +%H%M%S)}"
...
say "=== 接收端斷訊測試 RUN_ID=$RUN_ID domain=$ROS_DOMAIN_ID ==="
...
python3 -u evaluation/test_wb_cmd_chain.py >>"$LOG" 2>&1 \
  || fail "命令鏈純邏輯測試未通過"
python3 -u evaluation/test_wb_cutoff_check.py >>"$LOG" 2>&1 \
  || fail "判定器離線測試未通過 —— 不在判定器未驗證時開跑"
say "  純邏輯測試通過（命令鏈 + 判定器）"

say "[2/9] 啟動 Isaac 執行端（--mode sync）"
...
python3 -u evaluation/wb_cutoff_check.py --run "$DIR" 2>&1 | tee -a "$LOG"
CHECK=${PIPESTATUS[0]}
say "輸出 $DIR"
if [ "$CHECK" != "0" ]; then
  say "**判定未通過**（明細見 cutoff_check.json）"
  exit 6
fi
