...
if [ -z "${ROS_DOMAIN_ID:-}" ]; then
  ...
  echo "  ROS_DOMAIN_ID=95 bash evaluation/run_wb_e2.sh"
  exit 64
fi
export ROS_DOMAIN_ID            # 所有子程序沿用同一值

RUN_ID="${RUN_ID:-wb_e2_$(date +%H%M%S)}"
...
# ---- 2 準備階段與任務時間分開 ----
PROFILE_S=7.0                   # 零2 + 3 週期×1.0 + 零2
# **判準事前固定**，見 evaluation/results/specs/wb_e2_criteria_v1.yaml，
# 由 evaluation/wb_e2_check.py 離線判定。速率不得為了通過而調整。
ARM_RATE="${ARM_RATE:-0.05}"    # joint2，rad/s；與 arm 趟次同幅度
BASE_VX="${BASE_VX:-0.045}"     # ≤ 低速介面界限 0.05，**界限未動**
BASE_WZ="${BASE_WZ:-0.18}"      # ≤ 低速介面界限 0.2
CRITERIA="$WS/evaluation/results/specs/wb_e2_criteria_v1.yaml"
...
say "=== E2 輪級限制接入測試 RUN_ID=$RUN_ID domain=$ROS_DOMAIN_ID ==="
...
say "[2/8] 啟動 Isaac **E2** 執行端（--mode sync，含輪級限制）"
...
say "  幅度上限未動，改的是剖面形狀 —— 斜坡剖面 λ 恆為 1，驗不到限制器"
say "  起始底盤 yaw ${YAW}°（底盤命令在 report frame +x，非本體 +x）"
timeout 120 python3 -u evaluation/wb_trigger_cmd.py --out "$DIR" \
    --base-vx "$BASE_VX" --base-wz "$BASE_WZ" --arm-rate "$ARM_RATE" \
...
python3 -u evaluation/wb_e2_check.py --run "$DIR" 2>&1 | tee -a "$LOG"
CHECK=${PIPESTATUS[0]}
say "輸出 $DIR"
if [ "$CHECK" != "0" ]; then
  say "**判定未通過**（明細見 e2_check.json）"
  exit 6
fi
