...
if [ -z "${ROS_DOMAIN_ID:-}" ]; then
  ...
  echo "  ROS_DOMAIN_ID=95 bash evaluation/run_wb_e2_cutoff.sh"
  exit 64
fi
export ROS_DOMAIN_ID            # 所有子程序沿用同一值

RUN_ID="${RUN_ID:-wb_e2_cutoff_$(date +%H%M%S)}"
...
# **判準事前固定**，見 evaluation/results/specs/wb_e2_cutoff_criteria_v1.yaml，
# 由 evaluation/wb_e2_cutoff_check.py 離線判定。速率不得為了通過而調整。
ARM_RATE="${ARM_RATE:-0.05}"    # joint2，rad/s；與 arm 趟次同幅度
BASE_VX="${BASE_VX:-0.03}"      # report frame +x，m/s；與 base 趟次同幅度
CRITERIA="$WS/evaluation/results/specs/wb_e2_cutoff_criteria_v1.yaml"
...
say "=== **E2** 接收端斷訊測試 RUN_ID=$RUN_ID domain=$ROS_DOMAIN_ID ==="
...
ADAPTER_PID="${PIDS[-1]}"
say "  本趟 adapter PID=$ADAPTER_PID（切斷只對這一個 PID，切前核對 cmdline）"
# **獨立觀測程序**：證明 /wb_vel_cmd 實際停止更新，
# 不用執行端自己的 cmd_age 推論（那是同一個程序的內部狀態）。
spawn recorder python3 -u evaluation/wb_topic_recorder.py --out "$DIR"
sleep 5

say "[5/9] 起動前檢查（內容 / 新鮮度 / 端點身分）"
...
say "[7/9] 有界 sync 命令源＋切斷器（底盤 ${BASE_VX} m/s ＋ joint2 ${ARM_RATE} rad/s）"
say "  **同一則 9 維訊息同時帶兩個分量**，由同一個 scale 縮放"
say "  起始底盤 yaw ${YAW}°（底盤命令在 report frame +x，非本體 +x）"
say "  **切斷器先起**（偏移 ${CUT_OFFSET}s，自命令源第一則訊息起算）"
spawn cutter python3 -u evaluation/wb_cut_adapter.py \
    --pid "$ADAPTER_PID" --offset-s "$CUT_OFFSET" --out "$DIR"
...
say "[8/9] 等觀察窗（切斷 + ${OBSERVE_S}s 模擬時間）後檢查各程序存活"
[ -f "$DIR/cut_adapter.json" ] || fail "切斷器未產出 cut_adapter.json"
CUT_T=$(python3 -c "import json;print(json.load(open('$DIR/cut_adapter.json')).get('cut_sim_t','nan'))")
say "  切斷於 sim ${CUT_T}"
timeout 120 python3 -u evaluation/wb_wait_sim_t.py \
    --until "$CUT_T" --plus "$OBSERVE_S" 2>&1 | tee -a "$LOG"

# 各程序存活：只對本趟記錄的 PID 用 kill -0，**不做字串比對**
python3 - "$DIR" "${PIDS[0]}" "${PIDS[1]}" "${PIDS[2]}" "${PIDS[3]}" \
    "$ADAPTER_PID" "${PIDS[5]}" <<'LIVEPY' 2>&1 | tee -a "$LOG"
import json, os, sys
d = sys.argv[1]
names = ['isaac', 'rsp', 'dist', 'safety', 'adapter', 'recorder']
out = {}
for n, p in zip(names, sys.argv[2:]):
    try:
        os.kill(int(p), 0)
        out[n] = True
    except OSError:
        out[n] = False
json.dump(out, open(os.path.join(d, 'post_cut_liveness.json'), 'w'))
print('  存活：' + '  '.join(f'{k}={v}' for k, v in out.items()))
LIVEPY

# adapter 已刻意切斷：自清理清單移除，避免 PID 重用後誤殺他人程序
NEWP=(); NEWN=()
for n in "${!PIDS[@]}"; do
  if [ "${PIDS[$n]}" != "$ADAPTER_PID" ]; then
    NEWP+=("${PIDS[$n]}"); NEWN+=("${NAMES[$n]}")
  fi
done
PIDS=("${NEWP[@]}"); NAMES=("${NEWN[@]}")
say "  adapter PID $ADAPTER_PID 已自清理清單移除（已刻意切斷）"
...
say "[9/9] 離線判定（事前定版判準，不下修門檻）"
python3 -u evaluation/wb_e2_cutoff_check.py --run "$DIR" 2>&1 | tee -a "$LOG"
CHECK=${PIPESTATUS[0]}
say "輸出 $DIR"
if [ "$CHECK" != "0" ]; then
  say "**判定未通過**（明細見 e2_cutoff_check.json）"
  exit 6
fi
say "判定通過 —— **停下回報，不自動進輪級限制或 pregrasp**"
