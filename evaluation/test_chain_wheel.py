#!/usr/bin/env python3
"""鏈路測試：smoother + wheel_limit_guard，驗收點在 **guard 的輸出**。

**隔離**：使用獨立的 ROS_DOMAIN_ID 與測試專用 topic
（/t_cmd_in -> /t_cmd_mid），不接任何機器人命令入口，不啟動模擬器。
...
ap.add_argument('--out', default='evaluation/results/chain_wheel_test.json')
