...
座標約定（重要）
----------------
`/wholebody_safety/cmd_in` 的底盤三分量在**安全層的 `report_frame`**
（預設 `odom`）—— 那是濾波器解算所在的座標系。
**之後由 `arm_vel_adapter` 才轉成本體座標**再送給底盤。

所以本檔產生的是 **report frame 的 +x**，不是本體 +x。
兩者只有在底盤 yaw ≈ 0 時碰巧一致；**不能因為第一趟剛好對得上就混稱** ——
起始 yaw 由起動前檢查（`wb_preflight.py`）量測並記錄，本檔一併寫進輸出。
...
ap.add_argument('--base-vx', type=float, default=0.03,
                help='**report frame** +x 速度（m/s），不是本體 +x。'
                     '低速：執行端界限 0.05')
ap.add_argument('--expect-base-yaw-deg', type=float, default=None,
                help='起始底盤 yaw（度），由 wb_preflight 量得。'
                     '僅記錄用，讓 report frame 與本體的關係可查核')
...
def vector(scale):
    ...
    if a.mode in ('base', 'sync'):
        v[0] = a.base_vx * scale          # **report frame** +x（非本體 +x）
...
def main():
    ...
    print(f'[cmd] 幅度 base_vx {a.base_vx} m/s（**report frame +x**，非本體）、'
          f'arm_rate {a.arm_rate} rad/s', flush=True)
    if a.expect_base_yaw_deg is not None:
        print(f'[cmd] 起始底盤 yaw {a.expect_base_yaw_deg:+.3f}°'
              f'（yaw≈0 時 report frame 與本體碰巧一致，不可據此混稱）',
              flush=True)
    ...
    json.dump({'schema': 'wb_bounded_cmd/1', 'mode': a.mode, 'topic': TOPIC,
               ...
               'base_frame_convention': {
                   'cmd_in_base_components_in': 'safety report_frame（預設 odom）',
                   'converted_to_body_by': 'arm_vel_adapter',
                   'this_source_produces': 'report frame +x',
                   'start_base_yaw_deg': a.expect_base_yaw_deg,
                   'note': ('report frame 與本體只有在 yaw≈0 時一致；'
                            '不可因第一趟碰巧對上就混稱')},
               'rate_hz_nominal_wallclock': a.rate_hz,
               'timing_note': ('**剖面按模擬時間定義，發布節奏按牆鐘排程** —— '
                               '兩者分開記錄。不可稱為「模擬時間 20 Hz」；'
                               '實際的模擬時間間隔見 sent 的 sim_t 欄'),
               'n_sent': len(n.sent),
