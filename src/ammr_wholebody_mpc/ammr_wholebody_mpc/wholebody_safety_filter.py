...
class SafetyConfig:
    ...
    vmax: np.ndarray = field(default_factory=lambda: np.array(
        [0.2775, 0.2775, 1.1327, 3.14, 3.14, 3.14, 3.14, 3.14, 3.14]))
...
def _box_rows(cfg, n, cap):
    A, b = [], []
    vmax = np.minimum(cfg.vmax[:n], cap if np.isfinite(cap) else cfg.vmax[:n])
    for i in range(n):
        e = np.zeros(n)
        e[i] = 1.0
        A.append(e.copy()); b.append(float(vmax[i]))
        A.append(-e);       b.append(float(vmax[i]))
    return A, b
...
def filter_velocity(K, q, v_in, pts, cfg=None) -> SafetyResult:
